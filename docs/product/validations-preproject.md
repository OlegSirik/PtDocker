...
- [x] Gradle + `cel-java` compile/eval `io_age <= 75`
- [x] Маппинг переменных из `VariableContext.getValues()` (динамически, CEL `DYN`)
- [x] Unit-test: true/false → message (`CelRuleEngineTest`)

### Phase 1 — MVP (2 нед.)

- [x] Migration `V7__rules.sql`
- [x] Module `pt-rules`: entity, repository, `CelRuleEngine`, `RuleValidationService`
- [x] `RuleManagementController` CRUD + reload
- [x] Интеграция `PRE_QUOTE_VALIDATION` + `PRE_SAVE_VALIDATION` в orchestrator
- [x] Seed: `PAX_AGE_LIMIT` для tenant demo / product `AIR_PAX_COMBO`

### Phase 2 — Полный validation scope (1 нед.)

- [x] `POST_QUOTE_VALIDATION`, `POST_SAVE_VALIDATION` — в `ProcessOrchestratorService`
- [ ] Скрипт миграции старых `quoteValidator` → `pt_rules` (scope `PRODUCT`) — по необходимости
- [x] Admin UI в PoliTechFront (`/admin/rules`, редактор CEL)
...
**Статус:** Phase 0–2 реализованы. Backlog: миграция legacy `quoteValidator`, Phase 3 (UNDERWRITING, WORKFLOW, dry-run).
