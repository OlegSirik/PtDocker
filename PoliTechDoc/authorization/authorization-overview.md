...
### 7.2 Где НЕ вычисляется acting account

❌ Не допускается:
вычислять acting account в AuthorizationService
менять его в бизнес-сервисах
определять его на основе данных запроса (policy, claim и т.п.)
