...
SELECT 1, 'VSK', FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
SELECT 1, 1, 'ROOT', 'ROOT Client', FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
SELECT 1, 1, 1, NULL, 'ROOT', 'ROOT Account', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
SELECT 1, 1, 1, 'sys_admin', TRUE, 'SYS_ADMIN', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (
...
SELECT setval('acc_tenants_seq', (SELECT COALESCE(MAX(id), 1) FROM acc_tenants));
SELECT setval('acc_clients_seq', (SELECT COALESCE(MAX(id), 1) FROM acc_clients));
SELECT setval('acc_accounts_seq', (SELECT COALESCE(MAX(id), 1) FROM acc_accounts));
